...
# Oanda API Credentials should be encrypted
        ...
        try:
            ...
            if datetime.now().second % 30 == 15:
                if not inTrade() and oldTrade:
                    exeSqlInsert(
                        "INSERT INTO Tyler3_0_TradeHistory (PAIR, TIMEBUY, TIMESELL, PIPS, PIPFEE, SELL, BUYOPEN, "
                        "BUYHIGH, BUYLOW, BUYCLOSE, VOLUME, RSI, D, ADX,  MACDmag, "
                        "MACDdir, OBV, RSIprev, Dprev, ADXprev, MACDmagPrev, MACDdirprev, OBVprev, INITIALBALANCE, "
                        "FINALBALANCE, CERTAINTY) VALUES ( " +
                        "'" + str(buyConditions[0]) + "'" + ", " +
                        "'" + str(buyConditions[1]) + "'" + ", " +
                        "'" + str(datetime.now(pytz.timezone('America/Chicago'))) + "'" + ", " +
                        str(10000 * (getPrice(buyConditions[0]) - buyConditions[2])) + ", " +
                        str(10000 * (getPrice(buyConditions[0]) - buyConditions[2]) - getSpread(
                            buyConditions[0])) + ", " +
            ...
            if datetime.now().minute % 15 == 0 and datetime.now().second == 10:
                if marketOpen():
                    ...
                    for i in range(len(currencies)):
                        ...
                        checkRowPID(i, pid)
                        probabilities[i] = probabilityOfWinning(row)    # Find probability of winning for each row

                    print(
                        str(datetime.now(pytz.timezone('America/Chicago'))) + " EURUSD, GBPUSD, AUDUSD, NZDUSD: " + str(
                            probabilities))
                    write(
                        str(datetime.now(pytz.timezone('America/Chicago'))) + " EURUSD, GBPUSD, AUDUSD, NZDUSD: " + str(
                            probabilities))
                    # Decide if the probability of winning is high enough to enter a trade for each currency
                    currency, amount = decide(probabilities)
                    checkSpread(currency)
                    write("Currency: " + str(currency) + "  Amount: " + str(amount))
                    # Executes the trade if amount is 0 no order will occur
